...
public class PluginClassLoaderUtils {
    private static final Logger LOGGER = LoggerFactory.getLogger(PluginClassLoaderUtils.class);
    public static final String JDK_INTER_APP_CLASSLOADER = "jdk.internal.loader.ClassLoaders$AppClassLoader";

    public static void loadJar(String jarPath) {
        try {
            int javaVersion = getJavaVersion();
            ClassLoader classLoader = PluginClassLoaderUtils.class.getClassLoader();

            URL resource;
            if (jarPath.startsWith("http")) {
                resource = new URL(jarPath);
            } else {
                resource = PluginClassLoaderUtils.class.getClassLoader().getResource(jarPath);
            }
